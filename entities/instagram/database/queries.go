...
func InsertInstagramUser(db *pgx.Conn, ctx context.Context, user *InsertInstagramUserInput) (*InstagramUser, error) {
	var id int64
	row := db.QueryRow(ctx,
...
func SelectInstagramUserByUsername(db *pgx.Conn, ctx context.Context, username string) (*InstagramUser, error) {
...
func SelectInstagramUserById(db *pgx.Conn, ctx context.Context, id int64) (*InstagramUser, error) {
	var user InstagramUser
...
func InsertInstagramFollows(db *pgx.Conn, ctx context.Context, input *InsertInstagramFollowsInput) error {
...
func SelectInstagramFollowees(db *pgx.Conn, ctx context.Context, id int64) ([]InstagramUser, error) {
	followees := []InstagramUser{}
	rows, err := db.Query(ctx, "select user.id, user.kind, user.username, user.pfp_url from instagram.follows left join instagram.user on follows.followee_id = user.id where follows.follower_id = $1;", id)
	if err != nil {
...
func SelectInstagramFollowers(db *pgx.Conn, ctx context.Context, id int64) ([]InstagramUser, error) {
	followers := []InstagramUser{}
	rows, err := db.Query(ctx, "select user.id, user.kind, user.username, user.pfp_url from instagram.follows left join instagram.user on follows.follower_id = user.id where follows.followee_id = $1;", id)
	if err != nil {
